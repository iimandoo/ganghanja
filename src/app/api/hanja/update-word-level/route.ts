...
export async function PUT(request: NextRequest) {
  try {
    const { hanjaId, oldWord, newWord, vocabularyRange, userId } =
      await request.json();
    ...
    try {
      ...
      const { error: historyError } = await supabaseAdmin
        ...
        .insert({
          ...
          wordlevel: vocabularyRange,
          type: "modify",
          hanja: newWord.hanja, // 수정된 한자
          kor: newWord.kor, // 수정된 한글 음
          before_wordlevel_es: beforeData.wordlevel_es,
          before_wordlevel_mid: beforeData.wordlevel_mid,
          after_wordlevel_es: afterData.wordlevel_es,
          after_wordlevel_mid: afterData.wordlevel_mid,
...
export async function DELETE(request: NextRequest) {
  try {
    const { hanjaId, wordToDelete, vocabularyRange, userId } =
      await request.json();
    ...
    try {
      ...
      const { error: historyError } = await supabaseAdmin
        ...
        .insert({
          ...
          wordlevel: vocabularyRange,
          type: "delete",
          hanja: wordToDelete.hanja, // 삭제된 한자
          kor: wordToDelete.kor, // 삭제된 한글 음
          before_wordlevel_es: beforeData.wordlevel_es,
          before_wordlevel_mid: beforeData.wordlevel_mid,
          after_wordlevel_es: afterData.wordlevel_es,
          after_wordlevel_mid: afterData.wordlevel_mid,
